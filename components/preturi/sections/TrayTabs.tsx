    ...
    <div className={inline ? "" : "px-2 sm:px-3 lg:px-4 pb-2 sm:pb-3"}>
      ...
      <div className="flex items-center gap-1.5 sm:gap-2 overflow-x-auto overflow-y-hidden scrollbar-hide py-0.5 touch-manipulation">
        {quotes.map((q, index) => {
          ...
          <div key={q.id} className="relative group flex-shrink-0">
            <button
              ...
              className={`flex items-center gap-1.5 sm:gap-2 px-2.5 sm:px-3 lg:px-4 py-2 sm:py-2 min-h-11 md:min-h-0 rounded-md sm:rounded-lg text-xs sm:text-sm font-medium transition-all duration-200 whitespace-nowrap touch-manipulation
                ...
                ${(isVanzariPipeline || isReceptiePipeline) && quotes.length > 1 ? 'pr-14' : onEditTrayInline ? 'pr-9' : ''}`}
              ...
              <span className={`flex items-center justify-center w-5 h-5 rounded-full text-xs font-bold flex-shrink-0
                ${selectedQuoteId === q.id 
                  ...
                  : isVanzare
                    ...
                    : isUnassigned
                      ? 'bg-slate-400/30 text-slate-600 dark:text-slate-400'
                      : 'bg-slate-400/30 text-slate-600 dark:text-slate-400'
                }`}>
              ...
              <Popover open={isEditing} onOpenChange={(open) => {
                ...
                <PopoverTrigger asChild>
                  <button
                    ...
                    className={cn(
                      "absolute top-1/2 -translate-y-1/2 p-1.5 min-h-[32px] min-w-[32px] flex items-center justify-center rounded-full transition-all touch-manipulation",
            ...
            {(isVanzariPipeline || isReceptiePipeline) && quotes.length > 1 && (
              <button
                ...
                className="absolute right-1 top-1/2 -translate-y-1/2 p-1.5 min-h-[36px] min-w-[36px] flex items-center justify-center rounded-full bg-red-500/10 hover:bg-red-500/20 text-red-500 hover:text-red-600 transition-all opacity-100 md:opacity-0 md:group-hover:opacity-100 touch-manipulation"
                title="Șterge tăvița"
        ...
        {isReceptiePipeline && onCreateTrayInline ? (
          <div className="flex items-center gap-1.5 flex-shrink-0 rounded-lg border-2 border-dashed border-emerald-300 dark:border-emerald-700 bg-emerald-50/50 dark:bg-emerald-950/20 p-1.5 gap-2">
            <Input
              placeholder="Număr"
              value={inlineTrayNum}
        ...
        {/* Butonul "Trimite tăvițele" - disponibil în Recepție */}
        {shouldShowSendButton && (() => {
          // Calculează numărul de tăvițe care vor fi trimise (excluzând tăvițele de vânzare)
          const traysToSend = quotes.filter(q => q.number && q.number.trim() !== '' && !isVanzareTray(q.number))
          const vanzareTraysCount = quotes.filter(q => isVanzareTray(q.number)).length
          ...
          return (
            <button
              ...
              className="flex items-center gap-1.5 px-4 py-2 min-h-11 md:min-h-0 rounded-lg text-sm font-medium text-white shadow-md shadow-emerald-500/30 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed whitespace-nowrap flex-shrink-0 touch-manipulation"
              style={{
                backgroundColor: sendingTrays || traysToSend.length === 0 || traysAlreadyInDepartments 
                  ? 'rgb(16 185 129)' // emerald-500 static când disabled
                  : undefined,
                animation: sendingTrays || traysToSend.length === 0 || traysAlreadyInDepartments 
                  ? 'none' 
              ...
              title={
                sendingTrays 
                  ? "Se trimit tăvițele..." 
                  : traysToSend.length === 0 
                  ? vanzareTraysCount > 0 
                    ? "Toate tăvițele sunt de tip vânzare și nu necesită trimitere"
                    : "Nu există tăvițe de trimis" 
                  : traysAlreadyInDepartments 
                  ? "Tăvițele sunt deja trimise în departamente"
                  : `Trimite ${traysToSend.length} tăviț${traysToSend.length === 1 ? 'ă' : 'e'} în departamente${vanzareTraysCount > 0 ? ` (${vanzareTraysCount} tăviț${vanzareTraysCount === 1 ? 'ă' : 'e'} de vânzare exclus${vanzareTraysCount === 1 ? 'ă' : 'e'})` : ''}`
              }
            >
              {sendingTrays ? (
                <>
